...
int main(int argc, char const *argv[])
{
    ...
    for(int times=0;times<num_times;times++){
        ...
        for( i=0;i<M-2;i++){
            for( j=csc_row[i];j<csc_row[i+1];j++){
                if(csc_col[j]>i){
                    for( k=csc_row[csc_col[j]];k<csc_row[csc_col[j]+1];k++){
                        if(csc_col[k]>csc_col[j]){
                            for( l=csc_row[csc_col[k]];l<csc_row[csc_col[k]+1];l++){
                                if(csc_col[l]==i){
